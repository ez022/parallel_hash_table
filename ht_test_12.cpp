...
//test find;
    ...
    for (size_t i = 0; i < tablesize; i++) {
     ...
     std::cout << std::endl;
    std::cout << "--------------------"  << std::endl;
    printElement("val",slotwidth,separator);
    for (size_t i = 0; i < tablesize; i++) {
...
//     std::cout << "--------------------"  << std::endl;
    ...
    // std::cout << "now insert 83 back into the hashtable"  << std::endl;
    // myIntHashTable.insert(83);


    // std::cout << "now delete 83, 321, 25 in parallel"  << std::endl;


    // int myDeletes[3] = {83,321,25};

    // parallel_for(0, 3, [&](size_t i) {
    //     myIntHashTable.deleteVal(myDeletes[i]);
    // }); 
    // for (int i = 0; i < 3; i++) {
    //     //std::cout << "| " << myInserts[i]; 
    //     myIntHashTable.deleteVal(myDeletes[i]);
    ...
    std::cout << "now delete all in parallel"  << std::endl;


    int myDeletes[12] = {21, 62, 121,83, 145, 25, 65, 86, 321, 210, 63, 133};

    parallel_for(0, 12, [&](size_t i) {
        myIntHashTable.deleteVal(myDeletes[i]);
    }); 
